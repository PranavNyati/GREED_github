...
sed(const Data& g, const Data& h, std::vector< std::string > method_name, std::vector< std::string > method_args)
{
	...
	// adding query graph g to the environment
	...
	// adding target graph h to the environment
...
// PRANAV: Writing the ged function here on my own based on the sed function
// using Data = std::pair< std::vector< NodeLabel >, std::vector< std::pair< int, int >>>;

// std::tuple< double, double>
// ged(const Data& g, const Data& h, std::vector< std::string > method_name, std::vector< std::string > method_args)
// {
// 	ged::GEDEnv< int, NodeLabel, EdgeLabel > env;

// 	// adding query graph g to the environment
// 	auto gi = env.add_graph();
// 	const auto& g_x = g.first; // Node labels
// 	const auto& g_edge_index = g.second; // Edge indices
// 	for (int i = 0; i < (int)g_x.size(); ++i) {
// 		env.add_node(gi, i, g_x[i]);
// 	}
// 	for (const auto& p: g_edge_index) {
...
// 	// adding target graph h to the environment
// 	auto hi = env.add_graph();
// 	const auto& h_x = h.first; // Node labels
// 	const auto& h_edge_index = h.second; // Edge indices
// 	for (int i = 0; i < (int)h_x.size(); ++i) {
// 		env.add_node(hi, i, h_x[i]);
// 	}
// 	for (const auto& p: h_edge_index) {
