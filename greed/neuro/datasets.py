...
# sys.path.insert(0, '../../pyged/ext/gurobi911/linux64/lib')
import pyged
...
def random_bfs_sample(g, n_hops, trav_prob, node_mask=None, edge_mask=None):
    ...
    for _ in range(n_hops):
        ...
        # print(g.edge_index[0].shape)
        # print(front_mask)
        # print(random_mask.shape)
        # print(front_mask[g.edge_index[0]])
...
def inner_sed(queries, targets):
    ...
    if config.n_workers == 1:
        ...
        for i, j, sed_data in tqdm(map(sed_func, ((i,i) for i in range(n))), desc='pairs', total=n):
            lb[i], ub[j] = sed_data
    else:
        with mp.Pool(config.n_workers, sed_init, (p_queries, p_targets)) as p:
            ...
            tqdm.write(f'method_name: {config.method_name}')
            tqdm.write(f'method_args: {config.method_args}')
            for i, j, sed_data in tqdm(p.imap_unordered(sed_func, ((i,i) for i in range(n))), desc='pairs', total=n):
                lb[i], ub[j] = sed_data
...
def inner_sed_plus(queries, targets):
    ...
    if config.n_workers == 1:
        ...
        tqdm.write(f'method_name: {config.method_name}')
        tqdm.write(f'method_args: {config.method_args}')
        for i, j, sed_data in tqdm(map(sed_plus_func, ((i,i) for i in range(n))), desc='pairs', total=n):
        ...
        with mp.Pool(config.n_workers, sed_init, (p_queries, p_targets)) as p:
            ...
            tqdm.write(f'method_name: {config.method_name}')
            tqdm.write(f'method_args: {config.method_args}')
            for i, j, sed_data in tqdm(p.imap_unordered(sed_plus_func, ((i,i) for i in range(n))), desc='pairs', total=n):
                lb[i], ub[j], node_mask_idx, edge_mask_idx = sed_data
                node_masks[i] = torch.zeros(targets[i].num_nodes, dtype=torch.bool)
                node_masks[i][node_mask_idx] = True
                edge_masks[j] = torch.zeros(targets[j].num_edges, dtype=torch.bool)
...
def outer_sed(queries, targets):
    ...
    if config.n_workers == 1:
        ...
        tqdm.write(f'method_name: {config.method_name}')
        tqdm.write(f'method_args: {config.method_args}')
        for i, j, sed_data in tqdm(map(sed_func, it.product(range(nq), range(nt))), desc='pairs', total=nq*nt):
        ...
        with mp.Pool(config.n_workers, sed_init, (p_queries, p_targets)) as p:
            ...
            tqdm.write(f'method_name: {config.method_name}')
            tqdm.write(f'method_args: {config.method_args}')
            for i, j, sed_data in tqdm(p.imap_unordered(sed_func, it.product(range(nq), range(nt))), desc='pairs', total=nq*nt):
                lb[i,j], ub[i,j] = sed_data
...
#             ret.append(q)
    for _ in tqdm(range(n_queries), desc='sampled queries'): # make n_pairs number of queries (as many as the targets)
        while True:
            query = random_bfs_sample(random.choice(targets), n_hops, trav_prob)
...
def make_inner_dataset_plus(graphs, n_pairs, n_hops_query, trav_prob_query, node_lim_query=None, n_hops_target=None, targets=None):
    ...
    # if target is not specified, decompose the graphs if n_hops_target is specified, else use the graphs as targets
    if targets is None:
        targets = decompose(graphs, n_hops_target) if n_hops_target else graphs
    
    # queries are sampled from the targets themselves for n_pairs number of times
    queries = make_queries(targets, n_pairs, n_hops_query, trav_prob_query, node_lim_query)
    targets = random.choices(targets, k=n_pairs) # randomly sample n_pairs targets from the targets 
    
    # compute the lower and upper bounds on the distance metric (SED or GED) between the queries and the targets
