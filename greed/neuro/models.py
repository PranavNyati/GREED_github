...
class SiameseModel(torch.nn.Module):
    ...
    def forward(self, g, h):
        if self.weighted:
            ...
            # print("Siamese_model forward: gs shape=> ", self.gs.shape)
            self.hs = torch.tensor([x.num_nodes for x in h.to_data_list()], device=config.device)
            # print("Siamese_model forward: hs shape=> ", self.hs.shape)
        # print("Siamese_model forward: g shape=> ", g.x.shape)
        # print("Siamese_model forward: h shape=> ", h.x.shape)
...
class NormGEDModel(SiameseModel):
    ...
    def forward_emb(self, gx, hx):
        ...
        return torch.linalg.norm(gx-hx, dim=-1)
...
class NormSEDModel(SiameseModel):
    ...
    def forward_emb(self, gx, hx):
        ...
        return torch.linalg.norm(torch.nn.functional.relu(gx-hx), dim=-1)
