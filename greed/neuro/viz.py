...
def plot_inner_dataset(dataset, pred=None, n_items=None, random=False, **kwargs):
    ...
    for i in range(n_items):
        ...
        if pred is None:
            plt.suptitle(f'#{i}    Q: ({q.num_nodes}, {q.num_edges})    T: ({t.num_nodes}, {t.num_edges})    LB: {lb[i]:.2f}    UB: {ub[i]:.2f}')
        else:
            plt.suptitle(f'#{i}    Q: ({q.num_nodes}, {q.num_edges})    T: ({t.num_nodes}, {t.num_edges})    LB: {lb[i]:.2f}    UB: {ub[i]:.2f}    PRED: {pred[i]:.2f}    ERR: {err[i]:.2f}')
        plt.show()
        ipy.display.display(ipy.display.Markdown('---'))

def log_prediction_results(dataset, pred, random=False, logfile=None, n_items=None, **kwargs):   
    queries, targets, lb, ub = dataset
    if n_items is None:
        n_items = len(queries)
    ...
    print("NUMBER OF (QUERY, TARGET) PAIRS: ", n_items, file=logfile)
    print("====================================================", file=logfile)
    for i in range(n_items):
        print(f'#{i}  Q: ({queries[i].num_nodes}, {queries[i].num_edges})  T: ({targets[i].num_nodes}, {targets[i].num_edges})  LB: {lb[i]:.2f}  UB: {ub[i]:.2f}  PRED: {pred[i]:.2f}  ERR: {err[i]:.2f}', file=logfile)
    print("====================================================", file=logfile)
    # Average error
    print(f"\nAVERAGE ERROR OVER {n_items} PAIRS: {torch.mean(err):.2f}", file=logfile)
    
        
def save_results_to_csv(dataset, pred, n_items=None, random=False, csvpath=None, **kwargs):
    queries, targets, lb, ub = dataset
    if n_items is None:
        n_items = len(queries)
    ...
    with open(csvpath, 'w') as f:
        ...
        f.write("Pair Number, Query Nodes, Query Edges, Target Nodes, Target Edges, Lower Bound, Upper Bound, Prediction, Error\n")
        for i in range(n_items):
            f.write(f'{i}, {queries[i].num_nodes}, {queries[i].num_edges}, {targets[i].num_nodes}, {targets[i].num_edges}, {lb[i]:.2f}, {ub[i]:.2f}, {pred[i]:.2f}, {err[i]:.2f}\n')
...
def plot_inner_dataset_plus(dataset, metaset, n_items=None, random=False, **kwargs):
    ...
    for i in range(n_items):
        ...
        plt.suptitle(f'#{i}    LB: {lb[i]:.2f}    UB: {ub[i]:.2f}')
#         plt.savefig(f'{i}.png')
#         plt.close()
        plt.show()
        ipy.display.display(ipy.display.Markdown('---'))
...
def plot_outer_dataset(dataset, pred=None, n_items=None, random=False, **kwargs):
    ...
    for i in range(n_items):
        ...
        if pred is None:
            plt.suptitle(f'#{i}    Q: ({q.num_nodes}, {q.num_edges})    T: ({t.num_nodes}, {t.num_nodes})    LB: {lb[qi,ti]}    UB: {ub[qi,ti]:.2f}')
        else:
            plt.suptitle(f'#{i}    Q: ({q.num_nodes}, {q.num_edges})    T: ({t.num_nodes}, {t.num_nodes})    LB: {lb[qi,ti]}    UB: {ub[qi,ti]:.2f}    PRED: {pred[qi,ti]:.2f}    ERR: {err[qi,ti]:.2f}')
        plt.show()
        ipy.display.display(ipy.display.Markdown('---'))
